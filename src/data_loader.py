...
class SubsetSC(SPEECHCOMMANDS):
    def __init__(self, subset: str = None, root: str = "../SPEECHCOMMANDS"):
...
class PreprocessedSC(Dataset):
    def __init__(self, subset: str, root: str, sample_rate: int, label_list: list):
        self.dataset = SubsetSC(subset=subset, root=root)
        self.transform = torchaudio.transforms.Resample(
            orig_freq=self.dataset[0][1],  # original sample rate
    ...
    def __getitem__(self, idx):
        ...
        resampled = self.transform(waveform)
        input_values = resampled.squeeze(0)  # shape: [time]
        label_id = self.label_to_index[label]
        return {"input_values": input_values, "label": label_id}
...
def get_datasets(config):
    ...
    val_dataset = PreprocessedSC("validation", root, sample_rate, all_labels)
    test_dataset = PreprocessedSC("testing", root, sample_rate, all_labels)
    
    return train_dataset, val_dataset, test_dataset, all_labels
